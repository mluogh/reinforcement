...
def train():
    ...
    agent = DDPG(sess, [1, 1], obs_size, actor_learning_rate=0.0001, critic_learning_rate=0.001)
    ...
    for episode in range(FLAGS.num_episodes):
        ...
        while t < 300:
            ...
            # infer an action
            if episode > 2000:
                action = agent.get_action(np.reshape(state, (1, obs_size)), False)
            else:
                action = agent.get_action(np.reshape(state, (1, obs_size)), True)
            # print(action)
            # take it
